...
def filter_zipfiles_with_site_details():
    zipfiles_in_central = []
    for file in os.listdir(centraldir):
        if file.startswith(fsw) & file.endswith(".zip"):
            zipfiles_in_central.append(file)
    return zipfiles_in_central


# Delete real time timeout zip files in central Realtime directory after 10 minutes.
# List all files in central Realtime directory(central_realtime_path).
# Filter zip files with site_id and station_name in central Realtime directory.
...
def del_realtime_timeout_zipfiles():
    list_of_zipfiles = []
    allfiles = os.listdir(central_realtime_path)
    ...
    for i in range(0, len(list_of_zipfiles)):
        ...
        if minutes[0] > 2:
            try:
                zipfile_path = os.path.join(central_realtime_path, name_of_zipfile)
                print(zipfile_path)
                os.unlink(zipfile_path)
                print("File deleted in " + centraldir + "/Realtime- ", name_of_zipfile)
            except:
                print("Error while deleting file in " + centraldir + "/Realtime- ", name_of_zipfile)


# copies the files from central to Realtime
def move_zipfiles_to_realtime():
    zipfif = filter_zipfiles_with_site_details()
    for i in range(0, len(zipfif)):
        # Extracting zip file time
        zfln = zipfif[i]
        filepath = os.path.join(central_path, zipfif[i])
        fmin = int(zfln[-8:-6])
        fhrs = int(zfln[-10:-8])
        fday = int(zfln[-12:-10])
        fmon = int(zfln[-14:-12])
        ...
        if 0 <= minutes[0] <= 2:
            cpth = os.path.join(central_realtime_path, zfln)
            if os.path.isfile(cpth):
                # print("File exists")
                pass
            else:
                newPath = shutil.copy(filepath, central_realtime_path)
...
def zipfile_for_realtime_upload():
    try:
        ...
        for x in os.listdir(central_realtime_path):
            zipfiles.append(x)
        ...
        # print("No files in "+centraldir+"/"+rtdir+ " for realtime upload")
        return -2


# Read upload zipfile
def read_zipfile(zfln):
    filepath = os.path.join(central_realtime_path, zfln)
    f = open(filepath, "rb")
    while True:
        data = f.read()
        if not data:
...
def add_boundaries_to_zipfile_data(upload_zipfile_name):
    try:
        os.unlink("centralruf.txt")
        # print("File Deleted: centralruf.txt")
    except:
        pass
        # print("Error while deleting file: centralruf.txt")
...
Content-Type:application/zip\r\n\r\n'.format(upload_zipfile_name)
    ...
    file1 = open("centralruf.txt", "wb")  # append mode
...
# Reads data from centralruf.txt
def read_data_to_upload():
    fileName = "centralruf.txt"
    f = open(fileName, "rb")
    while True:
        data = f.read()
        if not data:
...
# Deletes the zip file from central/Realtime folder after getting Success message from the server.
def spcb_realtime_upload(upload_zipfile_name):
    auth, sign, ts = central_authorization()
    header = {
        'Host': central_hostAddress,
        'User-Agent': 'DTU',
    ...
    # Upload data
    f = open('centralruf.txt', 'rb')
    mydata = f.read()
    f.close()

    try:
        print("\n**************central Realtime upload****************")
        print("Uploaded Realtime File Name: ", upload_zipfile_name)
        resp = requests.post(central_realtime_url, headers=header, data=mydata, timeout=180)
        servermsg = json.loads(resp.text)
        print(servermsg)

        if servermsg["status"] == "success":
            print("\n*********central Realtime Upload Response**********")
            print("Successfully uploaded Realtime file Name: ", upload_zipfile_name)
            print(resp.text)
            #data = json.loads(resp.text)
            #print(data)
            try:
                rmzfit = os.path.join(centraldir, upload_zipfile_name)
                os.unlink(rmzfit)
                print("Deleted Real time uploaded file in " + centraldir + ": ", upload_zipfile_name)
            except FileNotFoundError:
                print("File already deleted")
            try:
                rmzfit = os.path.join(central_realtime_path, upload_zipfile_name)
                os.unlink(rmzfit)
                print("Deleted Real time uploaded file in " + centraldir + "/Realtime: ", upload_zipfile_name)
            except:
                print("Error while deleting Real time uploaded file in " + centraldir + "/Realtime: ", upload_zipfile_name)
        else:
            #print(servermsg)
            time.sleep(10)

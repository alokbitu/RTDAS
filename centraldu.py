...
# Filter zip files starts with site_id, station_name in central
...
# Copies the delayed zip files from central to Delayed.
# Deletes the copied zipfile in central.
def move_zipfiles_to_delayed():
    zipfif = filter_zipfiles_with_site_details()

    for i in range(0, len(zipfif)):
        # Extracting zip file time
        filepath = os.path.join(centraldir, zipfif[i])
        zfln = zipfif[i]
        ...
        if minutes[0] > 3:
            newPath = shutil.copy(filepath, central_delayed_path)
            try:
                # remove zip file in central
                rzfic = os.path.join(centraldir, zfln)
                os.unlink(rzfic)
                # print("File deleted in "+ centraldir +" - ", zfln)
            except:
                pass
                # print("Error while deleting file in "+ centraldir +" - ", zfln)
        else:
            pass
...
def zipfile_for_delayed_upload():
    try:
        ...
        for x in os.listdir(central_delayed_path):
            zipfif.append(x)
        ...
        # print("No files in "+ centraldir +"/"+dlydir+ " for delayed upload")
        return -1


# Reads zipfile in byte form and returns data
def read_zipfile(zfln):
    filepath = os.path.join(central_delayed_path, zfln)
    f = open(filepath, "rb")
    while True:
        data = f.read()
        if not data:
...
# After adding boundaries to zipfile data.  Creates centralduf.txt with added data.
def add_boundaries_to_zipfile_data(upload_zipfile_name):
    try:
        os.unlink("centralduf.txt")
        # print("File Deleted: centralduf.txt")
    except:
        print("Error while deleting file: centralduf.txt")
...
Content-Type:application/zip\r\n\r\n'.format(dzipfln)
    ...
    try:
        file1 = open("centralduf.txt", "wb")  # append mode
        file1.write(data.encode())
        file1.write(zipbuffer)
        file1.write(endb.encode())
        file1.close()
    except:
        dzf = os.path.join(central_delayed_path, dzipfln)
        os.unlink(dzf)


# Reads data from centralduf.txt and returns data
def read_data_to_upload():
    fileName = "centralduf.txt"
    f = open(fileName, "rb")
    while True:
        data = f.read()
        if not data:
...
# Deletes the zip file from central/Delayed folder after getting success message from the server.
def central_delayed_upload(upload_zipfile_name):
    ...
    try:
        ...
        if servermsg["status"] == "success":
            print("\n************central Delayed Upload Response**************")
            print("Successfully uploaded delayed File Name: ", upload_zipfile_name)
            print(resp.text)
            try:
                dzf = os.path.join(central_delayed_path, upload_zipfile_name)
                os.unlink(dzf)
